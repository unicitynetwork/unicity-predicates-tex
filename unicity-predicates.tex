...
\noindent For any fixed value of $L=(\sthash, \predi', \auxd', a)$, due to perfect hiding, commitments $\txhash=\commitc(H(\predi',x,\auxd'))$ and $\txhash'=\commitc(H(\predi',x'',\auxd'))$ have equal probability distributions. Moreover, by the Output Independence Lemma \cite{BTLR25} (with $g(x)=H(\predi',x,\auxd')$), the random variables $x$ and $\txhash=\commitc(H(\predi',x,\auxd'))$ are independent.
