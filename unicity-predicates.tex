...
\noindent\emph{System time} $\tau$ is a non-negative integer hold by $\unisrv$ and incremented when $R$ is certified by the BFT layer. 
...
\noindent\textbf{Name-specific arguments}: $u=((\sigma_1,j_1),\ldots,(\sigma_k,j_k))$ contain a list of pairs $(\sigma_i,j_i)$, where $j_i\in\{1,\ldots,k\}$ is the index of the public key that is supposed to be used for verifying $\sigma_i$. \medskip

\noindent\textbf{Definition}: $\nu_\mathsf{msig}(k,\pubkey_1,\ldots,\pubkey_n;\tau,m,u)=1$ iff: 
\begin{itemize}
...
The goal of atomic swap is to fairly and securely exchange tokens between their owners. If users $A$ and $B$ own tokens $\mathsf{Tok}_A$ and $\mathsf{Tok}_B$, respectively and they want to exchange their tokens so that after the exchange, $A$ owns $\mathsf{Tok}_B$ and vice versa. The parties agree to a certain timeout and run the swap protocol so that the following properties hold:
\begin{enumerate}
...
\item[2.] For $v'_\mathsf{swap}=H(\nu_\mathsf{swap}(\pubkey',h',\pubkey,h,\tau_\mathsf{max}),0^\ell,\bot)$ either (a) or (b): 
    \begin{itemize}
...
\noindent The function $f_\mathsf{inc}(k,v,\pi)\in\{0,1\}$ is evaluated via direct check $R[k]\stackrel{\mbox{?}}{=} v$ by $\unisrv$ and via the verification function $\mathcal{V}(k,v,\pi)$ by users.
...
\noindent\textbf{Preparation of $A$}: To transfer $\mathsf{Tok}_A$ to the state $(\nu^A_\mathsf{prep}(\pubkey_A;\cdot),h^1_A)$, the party $A$ runs the following protocol that involves communication with $\unisrv$:
\begin{enumerate}
    ...
    \item $\unisrv$ checks: $\nu_\mathsf{sig}(\pubkey_A;\tau^A_\mathsf{prep},H(h_A,v^A_\mathsf{prep}),u^A_\mathsf{prep},)=1$
...
$C^A_\mathsf{swap}=(T^A_\mathsf{swap},u^A_\mathsf{swap},v^A_\mathsf{swap},\pi^A_\mathsf{swap})$.
...
\end{enumerate}
$\mathsf{Tok}_B$ is now in the state $(\nu_\mathsf{sig}(\pubkey_A;\cdot),h^3_A)$, where $h^3_A=H(h^2_B,0^\ell)$, which means that $A$ has now full control over $\mathsf{Tok}_B$.  
\medskip
...
\begin{enumerate}
    ...
    \item $A$ computes: $u^A_\mathsf{fin}\gets (v^B_\mathsf{swap},\pi,\sigma^A_\mathsf{fin})$
...
Atomic swap protocol can also be desinged with the hashed timelock predicate \cite{Herl18,Bitc18a,Bitc18b}  $\nu_\mathsf{htlc}(\pubkey,\pubkey',y,\tau_\mathsf{max};\cdot)$ as follows: 
\begin{enumerate}
...
The Unicity Execution Layer. A manuscript (2025)
